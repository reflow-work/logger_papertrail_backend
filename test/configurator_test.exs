defmodule LoggerPapertrailBackend.ConfiguratorTest do
  ...
  test "should be able to use host-type configuration" do
    ...
    assert %Configuration{host: "somehost", port: 2810, system_name: "app"} ==
             Configurator.configure_papertrail_target(config)
  end

  test "should be able to use url-format with papertrail as schema" do
    config = [url: "papertrail://somehost:667/my_app", level: :info]

    assert %Configuration{host: "somehost", port: 667, system_name: "my_app"} ==
             Configurator.configure_papertrail_target(config)
  end
  ...
  test "host-config from readme should work" do
    ...
    assert %Configuration{host: "logs.papertrailapp.com", port: 11, system_name: "Wizard"} ==
             Configurator.configure_papertrail_target(config)
  end
  ...
  test "system_name is optional in host config" do
    ...
    assert %Configuration{host: "somehost", port: 22, system_name: nil} ==
             Configurator.configure_papertrail_target(config)
  end
